   ...
   : sign(1)
{
   ...
   int oursign = 1; 
      // We keep the sign and add it later.

   if ( *it == '-' )
   {
      oursign = -1;
      ++ it; 
      std::cout << "recognized minus\n";
   }
   else
   {
      if( *it == '+' )
   ...
   constexpr long int maxpow10 = 1000000000000000; 
   ...
   while( it != s. end( )) 
   {
      ...
      if( !isdigit(c))
         throw std::invalid_argument( 
                         "cannot construct bigint, non-digit found" );
...
bool bigint::testmachine( )
{
   ...
   static_assert( sizeof( long int ) >= 8, "long int not long enough" );
...
long unsigned int 
bigint::checksum( long unsigned int prime ) const
{
