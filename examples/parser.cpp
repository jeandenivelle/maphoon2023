...
symbol 
parser::parse( symboltype start )
{
   std::cout << "hello\n";
  
   const auto& st = startsymbols. at( start );

   std::cout << "found it\n";
   statestack. push_back( st. first );
...
restart:
   ...
   auto def = defaultreductions. find( statestack. back( )); 
   if( def != defaultreductions. end( ) &&
       def -> second. second )
   {
      std::cout << "the state is trivial" ;
   }
   else
   {
      std::cout << "this state is not trival, we will need a lookahead\n";
...
#endif
      // We check if we are in an accepting state. 
...
$C$
   ...
   std::unordered_map< symboltype, std::pair< size_t, std::unordered_set< symboltype >>> 
   parser::startsymbols =
   {
      { sym_Stat, { 0, { sym_EOF }} }
   };
...
std::unordered_map< size_t, std::pair< size_t, bool >>
parser::defaultreductions =
   { { 1, {2,false} },  { 3, {4,true} }};

std::unordered_map< parser::statesymbol, parser::decision,
                    parser::statesymbol::hash, parser::statesymbol::equal_to >
parser::decisiontable = { { { 0, sym_E }, { 1,2,3 } } };

std::vector< size_t > parser::redtable = { 100,101,102};
